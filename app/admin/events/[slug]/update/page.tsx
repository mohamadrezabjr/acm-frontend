...
export default function EditEventPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        setEventData({
          ...
          slug: eventDataFromAPI.slug || "",
          description: eventDataFromAPI.description || "",
          startDateTime: eventDataFromAPI.start_date ? new DateObject(new Date(eventDataFromAPI.start_date)) : null,
          endDateTime: eventDataFromAPI.end_date ? new DateObject(new Date(eventDataFromAPI.end_date)) : null,
          registrationStartDateTime: eventDataFromAPI.registration_start_at
            ? new DateObject(new Date(eventDataFromAPI.registration_start_at))
            : null,
          registrationDeadlineDateTime: eventDataFromAPI.registration_deadline
            ? new DateObject(new Date(eventDataFromAPI.registration_deadline))
            : null,
          location: eventDataFromAPI.location || "",
          capacity: eventDataFromAPI.capacity?.toString() || "",
          organizer: eventDataFromAPI.organizer || "",
          price: eventDataFromAPI.price?.toString() || "",
          image: null,
          existingImageUrl: eventDataFromAPI.image || "",
        })

        // Set tags - convert string array to Tag objects
        if (eventDataFromAPI.tags && Array.isArray(eventDataFromAPI.tags)) {
          const mappedTags = eventDataFromAPI.tags.map((tagName: string, index: number) => ({
            id: Date.now() + index,
            name: tagName,
          }))
        ...
        // Set speakers
        if (eventDataFromAPI.speakers) {
          const mappedSpeakers = eventDataFromAPI.speakers.map((speaker: any, index: number) => ({
            id: `${speaker.id || Date.now()}-${index}`,
            person_id: speaker.id || null,
            type: speaker.id ? "existing" : "new",
            user: speaker.user || null,
  ...
  const handleRemoveImage = () => {
    setEventData({ ...eventData, image: null, existingImageUrl: "" })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("رویداد با موفقیت ویرایش شد!")
      router.push("/admin/dashboard")
    } catch (error) {
      console.error("Error updating event:", error)
      alert("خطا در ویرایش رویداد")
    } finally {
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">ویرایش رویداد</CardTitle>
            <CardDescription>اطلاعات رویداد را ویرایش کنید</CardDescription>
          </CardHeader>
          <CardContent>
